...
- (void)_setWheelView {
    //中间平铺
    wheelCenterView=[[UIImageView alloc]initWithFrame:CGRectMake(0,0,self.frame.size.width ,self.frame.size.height)];
...
-(void)setFrame:(CGRect)frame{
    ...
    wheelCenterView.frame = CGRectMake(0, 0, frame.size.width, frame.size.height);
    
    CGFloat rowHeight=wheelCenterView.frame.size.height/_cellCountInVisible;
    CGFloat selectionY=rowHeight*_selectionPosition+3;
    
    selectedCenterView.frame = CGRectMake(0,selectionY,self.frame.size.width,rowHeight);
    int i = 0;
    for(UIScrollView *view in _scrolls){
        ...
        CGFloat height=wheelCenterView.frame.size.height/_cellCountInVisible*_cellCountInVisible;
        view.frame = CGRectMake(0+x+2, 0+3, [[_scrollWidthProportion objectAtIndex:i]floatValue]*wheelCenterView.bounds.size.width, height);
        i++;
    }
...
- (void)_setSelectionArea {
    ...
    //中间平铺
    UIImageView *selectionCenterView=[[UIImageView alloc]initWithFrame:CGRectMake(0,selectionY,self.frame.size.width,rowHeight)];
    selectionCenterView.backgroundColor = [UIColor colorWithRed:132.0/255.0 green:206.0/255.0 blue:253.0/255.0 alpha:1.0];
    selectionCenterView.alpha=0.5;
    selectedCenterView = selectionCenterView;
