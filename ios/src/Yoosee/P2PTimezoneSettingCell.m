...
@implementation P2PTimezoneSettingCell
...
-(void)layoutSubviews{
    ...
    if(!self.customView){
        ...
        TimezoneView *timezoneView = [[TimezoneView alloc] initWithFrame:CGRectMake(30,5, cellWidth-30*2, cellHeight-5*2)];
        timezoneView.delegate = self;
        self.customView = timezoneView;
        ...
        [self.customView setFrame:CGRectMake(30,5, cellWidth-30*2, cellHeight-5*2)];
...
-(void)onTimezoneChange:(NSInteger)timezone{
    if(nil!=self.OnTimezoneChange){
        self.OnTimezoneChange(timezone);
    }
}
@end
