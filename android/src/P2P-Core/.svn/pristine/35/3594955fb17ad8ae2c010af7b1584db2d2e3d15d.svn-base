...
public class P2PView extends BaseP2PView {
	...
	private void vSetWindow() {
		...
		Log.i("dxsp2pview", "fgFullScreen" + fgFullScreen);
		if (fgFullScreen == 0) {
			if(type==1){
				if (scale==0) {
					int Rate, Rate2;
		...
		Log.i("dxsview", "mWidth:" + mWidth + "mHeight" + mHeight
				+ "fgFullScreen" + fgFullScreen);
		if (null != mPlayer) {
	...
	SurfaceHolder.Callback mSHCallback = new SurfaceHolder.Callback() {
		public void surfaceChanged(SurfaceHolder holder, int format, int w,
			...
			Log.e("dxsdxs", "Window size:" + w + "x" + h + "format" + format);
