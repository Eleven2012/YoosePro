...
public class P2PHandler {
	...
	private static int MSG_ID_SETTING_ALARM_EMAIL = (int) (Constants.MsgSection.MSG_ID_SETTING_ALARM_EMAIL + Math.random()*999);
	private static int MSG_ID_GETTING_ALARM_EMAIL = (int) (Constants.MsgSection.MSG_ID_GETTING_ALARM_EMAIL+ Math.random()*999);
	...
	public boolean p2pConnect(String activeUser, int codeStr1, int codeStr2) {
		...
		if (activeUser.equals("517400")) {
			connect = MediaPlayer.getInstance().native_p2p_connect(
					...
					codeStr2, cHostName.getBytes(), Config.AppConfig.CustomerIDs);
		} else {
			connect = MediaPlayer.getInstance().native_p2p_connect(
					Integer.parseInt(activeUser) | 0x80000000, 886976412,
					codeStr1, codeStr2, cHostName.getBytes(), Config.AppConfig.CustomerIDs);
		}
		if (connect == 1) {
			return true;
		} else {
	 ...
	 * @param port 发件人的邮局SMTP端口参数
	 * @param server 发件人的邮局SMTP服务参数
	 * @param user 发件人邮箱
	 * @param pwd 发件人邮箱密码
	 * @param subject 发件主题
	 * @param content 发件邮件内容
	 */
	public void setAlarmEmailWithSMTP(String contactId, String password, byte boption,String emailaddress,int port,String server,String user,String pwd,String subject,String content) {
		Log.e(TAG, "P2PHANDLER:setAlarmEmail");
		if (this.MSG_ID_SETTING_ALARM_EMAIL >= (Constants.MsgSection.MSG_ID_SETTING_ALARM_EMAIL)) {
			this.MSG_ID_SETTING_ALARM_EMAIL = Constants.MsgSection.MSG_ID_SETTING_ALARM_EMAIL - 1000;
		}
		...
		MediaPlayer.SetRobortEmailNew(Integer.parseInt(contactId),
				Integer.parseInt(password), this.MSG_ID_SETTING_ALARM_EMAIL,
				boption, emailaddress, port, server, user, ppp, subject, content, ppp.length);
		this.MSG_ID_SETTING_ALARM_EMAIL++;
	}
	...
	public void playbackConnect(String contactId, String password,
		...
		MediaPlayer.getInstance().native_p2p_call(Integer.parseInt(contactId),
				...
				Integer.parseInt(password), recordFilePosition, byt,
				"".getBytes());
	}
	...
	public void getAlarmCenterParameters(String contactId,String password){
		...
		Log.e("alarmcenter","MESG_GET_ALARM_CENTER_PARAMETER="+this.MESG_GET_ALARM_CENTER_PARAMETER+"  "+"length="+datas.length);
		MediaPlayer.iExtendedCmd(Integer.parseInt(contactId),
				Integer.parseInt(password), 
	 ...
	 * @param state
	 */
	public void setAlarmCenterParameters(String contactId,String password,int port,String ipdress,String userId,int state){
	    ...
	    int [] ips={Integer.parseInt(ipdesss[0]),Integer.parseInt(ipdesss[1]),Integer.parseInt(ipdesss[2]),Integer.parseInt(ipdesss[3])};
...
//	    datas[23]=(byte)(user_id2>>56&0xFF);
	    ...
	    Log.e("alarmcenter","MESG_SET_ALARM_CENTER_PARAMETER="+this.MESG_SET_ALARM_CENTER_PARAMETER+"  "+"length="+datas.length);
		MediaPlayer.iExtendedCmd(Integer.parseInt(contactId),
				Integer.parseInt(password), 
