...
public class ModifyNpcVisitorPasswordActivity extends BaseActivity implements
	...
	public void initComponent() {
		...
		if (visitorPwd == null || visitorPwd.length() <= 0||visitorPwd.equals("0")) {
...
private String visitor_password;
	...
	public void onClick(View v) {
		...
		case R.id.back_btn:
			...
			finish();
			break;
		case R.id.btn_clear_visitorpwd:
			// 清除访客密码
			showClearEmail();
			break;
		case R.id.save:
			...
			if (visitor_password.length() >= 10) {
				T.showShort(mContext, R.string.visitor_pwd_to_long);
				return;
			}
			if (!Utils.isNumeric(visitor_password)
					|| visitor_password.charAt(0) == '0') {
