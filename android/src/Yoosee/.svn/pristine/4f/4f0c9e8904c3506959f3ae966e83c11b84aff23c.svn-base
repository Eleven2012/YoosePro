...
//		}	
	...
	public void loadMusicAndVibrate(){
		isAlarm = true;
		int a_muteState = SharedPreferencesManager.getInstance().getAMuteState(MyApp.app);
		if(a_muteState==1){
			MusicManger.getInstance().playAlarmMusic();
		}
		
		int a_vibrateState = SharedPreferencesManager.getInstance().getAVibrateState(MyApp.app);
		if(a_vibrateState==1){
			new Thread(){
				public void run(){
					while(isAlarm){
	...
	public void initComponent(){
		...
		final AnimationDrawable anim = (AnimationDrawable) alarm_img.getDrawable();
	    ...
	    alarm_dialog.startAnimation(AnimationUtils.loadAnimation(mContext, R.anim.slide_in_right));
	...
	public void onClick(View v) {
		...
		case R.id.ignore_btn:
			int timeInterval = SharedPreferencesManager.getInstance().getAlarmTimeInterval(mContext);
			T.showShort(mContext, 
					mContext.getResources().getString(R.string.ignore_alarm_prompt_start)+" "+
					timeInterval+" "+
					mContext.getResources().getString(R.string.ignore_alarm_prompt_end));
			finish();
			break;
		case R.id.monitor_btn:
			...
			isOpendoor=false;
			final Contact contact = FList.getInstance().isContact(String.valueOf(contactId));
			if(null!=contact){
				hasContact = true;
				P2PConnect.vReject("");
				new Thread(){
					public void run(){
						while(true){
							if(P2PConnect.getCurrent_state()==P2PConnect.P2P_STATE_NONE){
								...
								String[] data = new String[]{contact.contactId,contact.contactPassword};
								msg.obj = data;
			...
			if(!hasContact){
				...
				anim.setAnimationListener(new AnimationListener(){
					...
					public void onAnimationEnd(Animation arg0) {
						// TODO Auto-generated method stub
						InputMethodManager m = (InputMethodManager) alarm_input.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
		...
		case R.id.alarm_go:
			...
			if(!isOpendoor){
				...
				new Thread(){
					public void run(){
						while(true){
							if(P2PConnect.getCurrent_state()==P2PConnect.P2P_STATE_NONE){
								Message msg = new Message();
								String[] data = new String[]{String.valueOf(contactId),password};
								msg.obj = data;
								handler.sendMessage(msg);
								break;
							}
				...
				P2PHandler.getInstance().sendCustomCmd(contactId, password, open_door_order);
				P2PHandler.getInstance().setGPIO1_0(contactId, password);
				finish();
			}
			break;
		case R.id.shield_btn:
		     Contact mcontact = FList.getInstance().isContact(String.valueOf(contactId));
		     String open_door_order="IPC1anerfa:unlock";
			 if(mcontact!=null){
				P2PHandler.getInstance().sendCustomCmd(mcontact.contactId,mcontact.contactPassword, open_door_order);
				P2PHandler.getInstance().setGPIO1_0(mcontact.contactId, mcontact.contactPassword);
				finish();
			 }else{
				 isOpendoor=true;
	...
	protected void onStop() {
		// TODO Auto-generated method stub
		super.onStop();
		SharedPreferencesManager.getInstance().putIgnoreAlarmTime(mContext, System.currentTimeMillis());
