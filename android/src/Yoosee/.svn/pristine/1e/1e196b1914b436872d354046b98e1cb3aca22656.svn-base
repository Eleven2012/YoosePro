...
public class SettingSdBellActivity extends BaseActivity implements OnClickListener{
	...
	public void playMusic(int bellId){
		...
		try {
			player.reset();
			bellType = SharedPreferencesManager.getInstance().getCBellType(context);
			HashMap<String,String> data;
			data = SystemDataManager.getInstance().findSdBellById(context, bellId);
	...
	public void onClick(View view) {
		...
		case R.id.save:
			if(checkedId==-1){
				 ...
				 if(settingType==SettingSystemActivity.SET_TYPE_COMMING_RING){
					 SharedPreferencesManager.getInstance().putCSdBellId(checkedId, this);
					 SharedPreferencesManager.getInstance().putCBellSelectPos(selectPos, this);
					 SharedPreferencesManager.getInstance().putCBellType(SharedPreferencesManager.TYPE_BELL_SD, this);
					 Intent i = new Intent();
					 i.setAction(SettingSystemActivity.ACTION_CHANGEBELL);
					 sendBroadcast(i);
				 }else if(settingType==SettingSystemActivity.SET_TYPE_ALLARM_RING){
					 SharedPreferencesManager.getInstance().putASdBellId(checkedId, this);
					 SharedPreferencesManager.getInstance().putABellSelectPos(selectPos, this);
					 SharedPreferencesManager.getInstance().putABellType(SharedPreferencesManager.TYPE_BELL_SD, this);
					 Intent i = new Intent();
					 i.setAction(SettingSystemActivity.ACTION_CHANGEBELL);
					 sendBroadcast(i);
				 }
