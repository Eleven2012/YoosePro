...
public class ModifyLoginPasswordActivity extends BaseActivity implements OnClickListener{
	...
	public void onClick(View view) {
		...
		case R.id.save:
			...
			new ModifyLoginPasswordTask(
					NpcCommon.mThreeNum,
					account.sessionId,
					password_old,
					password_new,
					password_re_new).execute();
	...
	class ModifyLoginPasswordTask extends AsyncTask{
		...
		protected Object doInBackground(Object... params) {
			...
			return NetManager.getInstance(mContext).modifyLoginPassword(threeNum, sessionId, oldPwd, newPwd, rePwd);
		}
		...
		protected void onPostExecute(Object object) {
			...
			case NetManager.CONNECT_CHANGE:
				new ModifyLoginPasswordTask(threeNum, sessionId, oldPwd, newPwd, rePwd).execute();
				return;
