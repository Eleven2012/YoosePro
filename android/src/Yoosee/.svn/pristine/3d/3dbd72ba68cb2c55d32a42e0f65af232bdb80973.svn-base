...
public class RegisterActivity extends BaseActivity implements  OnClickListener {
	...
	private void getPhoneCode(){
		...
		new GetPhoneCodeTask(count.substring(1,count.length()),phone).execute();
	...
	class GetPhoneCodeTask extends AsyncTask{
		...
		protected void onPostExecute(Object object) {
			...
			case NetManager.GET_PHONE_CODE_SUCCESS:
				...
				if(!isDialogCanel){
					if(CountryCode.equals("86")){
						Intent i = new Intent(mContext,VerifyPhoneActivity.class);
			...
			case NetManager.GET_PHONE_CODE_PHONE_FORMAT_ERROR:
				...
				if(!isDialogCanel){
					Utils.showPromptDialog(mContext,R.string.prompt, R.string.phone_format_error);
				}
				break;
			...
			default:
				...
				if(!isDialogCanel){
					Utils.showPromptDialog(mContext,R.string.prompt, R.string.registerfail);
				}
				break;
			}
		}
