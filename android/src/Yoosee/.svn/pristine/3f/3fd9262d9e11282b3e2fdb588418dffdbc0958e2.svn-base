...
public class SettingBellRingActivity extends BaseActivity implements OnClickListener{
	...
	public void initCompent() {
		...
		initSelectState();
		ArrayList<HashMap<String,String>> bells = SystemDataManager.getInstance().getSysBells(this);
	...
	public void initSelectState(){
		if(settingType==SettingSystemActivity.SET_TYPE_COMMING_RING){
			selectPos = SharedPreferencesManager.getInstance().getCBellSelectPos(this);
			bellType = SharedPreferencesManager.getInstance().getCBellType(this);
			
			if(bellType==SharedPreferencesManager.TYPE_BELL_SYS){
				checkedId = SharedPreferencesManager.getInstance().getCSystemBellId(this);
				selectBell.setText("");
			}else{
				checkedId = SharedPreferencesManager.getInstance().getCSdBellId(this);
				HashMap<String,String> data = SystemDataManager.getInstance().findSdBellById(context, checkedId);
				if(null!=data){
	...
	public void onClick(View view) {
		...
		case R.id.save:
			 if(checkedId==-1){
				 ...
				 if(settingType==SettingSystemActivity.SET_TYPE_COMMING_RING){
					 SharedPreferencesManager.getInstance().putCSystemBellId(checkedId, this);
					 SharedPreferencesManager.getInstance().putCBellSelectPos(selectPos, this);
					 SharedPreferencesManager.getInstance().putCBellType(SharedPreferencesManager.TYPE_BELL_SYS, this);
					 Intent i = new Intent();
					 i.setAction(SettingSystemActivity.ACTION_CHANGEBELL);
					 sendBroadcast(i);
				 }else if(settingType==SettingSystemActivity.SET_TYPE_ALLARM_RING){
					 SharedPreferencesManager.getInstance().putASystemBellId(checkedId, this);
					 SharedPreferencesManager.getInstance().putABellSelectPos(selectPos, this);
					 SharedPreferencesManager.getInstance().putABellType(SharedPreferencesManager.TYPE_BELL_SYS, this);
					 Intent i = new Intent();
					 i.setAction(SettingSystemActivity.ACTION_CHANGEBELL);
					 sendBroadcast(i);
				 }
	...
	public void playMusic(int bellId){
		...
		try {
			...
			data = SystemDataManager.getInstance().findSystemBellById(context, bellId);
