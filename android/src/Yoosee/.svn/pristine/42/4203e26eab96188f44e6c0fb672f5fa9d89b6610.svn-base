...
public class ContactDB {
	...
	public void update(Contact contact) {
		...
		values.put(COLUMN_ACTIVE_USER, contact.activeUser);
		try {
			myDatabase.update(TABLE_NAME, values, COLUMN_ACTIVE_USER + "=? AND "+COLUMN_CONTACT_ID + "=?",
					new String[] { contact.activeUser,contact.contactId});
		} catch (SQLiteConstraintException e) {
			e.printStackTrace();
		}
	...
	public List<Contact> findByActiveUserId(String activeUserId) {
		...
		if (cursor != null) {
			while (cursor.moveToNext()) {
				...
				lists.add(data);
				Log.e("dxsdoorbell", data.contactId+"type-db-->"+data.contactType);
			}
			cursor.close();
		}
		return lists;
