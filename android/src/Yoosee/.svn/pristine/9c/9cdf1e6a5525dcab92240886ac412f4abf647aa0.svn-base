...
public class RegisterActivity2 extends BaseActivity implements OnClickListener{
	...
	private void register(){
		...
		dialog.showDialog();
		if(isEmailRegister){
			new RegisterTask("1",email_str,count,"",pwd_str,confirm_pwd_str,code,"1").execute();
		}else{
			if(count.equals("86")){
				new RegisterTask("1","",count,phone,pwd_str,confirm_pwd_str,code,"1").execute();
			}else{
				new RegisterTask("1",email_str,count,phone,pwd_str,confirm_pwd_str,"","1").execute();
			}
		}
	...
	class RegisterTask extends AsyncTask{
		...
		protected Object doInBackground(Object... params) {
			...
			return NetManager.getInstance(context).register(
					VersionFlag,
					Email,
					CountryCode,
					PhoneNO,
		...
		protected void onPostExecute(Object object) {
			...
			case NetManager.REGISTER_EMAIL_USED:
				...
				if(!isDialogCanel){
					Utils.showPromptDialog(context,R.string.prompt, R.string.email_used);
				}
				break;
			case NetManager.REGISTER_EMAIL_FORMAT_ERROR:
				...
				if(!isDialogCanel){
					Utils.showPromptDialog(context,R.string.prompt, R.string.email_format_error);
				}
				break;
