...
public class MonitorActivity extends BaseMonitorActivity implements
	...
	private Button btnRecodingTest;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.i("dxsmonitor", "contactType-->" + contactType
				+ "isSurpportOpenDoor-->" + isSurpportOpenDoor);
	}
	...
	public void initComponent() {
		...
		btnRecodingTest=(Button) findViewById(R.id.btn_recoding);
		btnRecodingTest.setOnClickListener(new OnClickListener() {
     ...
     * @param callId
     */
    private void startMoniterRecoding(String callId,View v) {
        String pathName = Utils.getVideoRecodeName(callId);
        if (pathName.equals("noSD")) {
            T.show(mContext, R.string.sd_no_exist, 2000);
            return;
        }
        MediaPlayer.getInstance().setAVFilePath(pathName);
        if (MediaPlayer.getInstance().startRecoder() == 1) {
            P2PHandler.getInstance().setRecvAVDataEnable(true);
            isRecoding = true;
            ((Button)(v)).setText("录像中...");
        } else {
    ...
    private void stopRecoding(View v){
    	isRecoding = false;
        int isRecodingSuccess = MediaPlayer.getInstance().stopRecoder();
        P2PHandler.getInstance().setRecvAVDataEnable(false);
        if (isRecodingSuccess == 0) {
            T.show(mContext, "Error", 2000);
        } else {
            T.show(mContext, "stop recoding", 2000);
        }
        ((Button)v).setText("录像");
    }
			...
			if (intent.getAction().equals(Constants.P2P.P2P_REJECT)) {
				...
				if (contactId.equals(callId)) {
					if (state == Constants.P2P_SET.REMOTE_DEFENCE_SET.ALARM_SWITCH_ON) {
						isDefenceOn = true;
						defence_state
								.setBackgroundResource(R.drawable.deployment);
					} else {
						isDefenceOn = false;
