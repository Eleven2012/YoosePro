...
public class IpcListAdapter extends BaseAdapter{
	...
	public View getView(int position, View convertView, ViewGroup arg2) {
		...
		if(null==convertView){
			...
			convertView = LayoutInflater.from(context).inflate(R.layout.list_ipc_item, null);
			viewHoler.headerImg  =(HeaderView)convertView.findViewById(R.id.header_img);
			viewHoler.typeImg =(ImageView)convertView.findViewById(R.id.type_img);
			viewHoler.nameText=(TextView)convertView.findViewById(R.id.ipc_name);
			viewHoler.operatorImg=(ImageView)convertView.findViewById(R.id.operator_img);
			viewHoler.main = (RelativeLayout) convertView.findViewById(R.id.main); 
			viewHoler.addressText = (TextView)convertView.findViewById(R.id.ipc_address);
		...
		if(position==data.size()-1&&isShowAnim){
			convertView.startAnimation(AnimationUtils.loadAnimation(context, R.anim.ipc_item_down));
		}
...
//						addContact(threeNum,deviceType,true,ipFlag);
						...
						Log.e("contact","contactpassword"+contact.contactPassword);
					}
				}else{
					if(flag==Constants.DeviceFlag.ALREADY_SET_PASSWORD){
