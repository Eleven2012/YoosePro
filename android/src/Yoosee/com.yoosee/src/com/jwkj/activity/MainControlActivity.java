...
public class MainControlActivity extends BaseActivity implements OnClickListener{
	...
	private String[] fragTags = new String[]{
			"mainFrag","timeFrag","remoteFrag",
			"loadFrag","faultFrag","alarmFrag",
			"videoFrag","recordFrag","securityFrag",
			"netFrag","defenceAreaFrag","sdCardFrag"};
		...
		public void onReceive(Context arg0, Intent intent) {
			...
			if(intent.getAction().equals(Constants.Action.CONTROL_SETTING_PWD_ERROR)){
				...
				if(result==Constants.P2P_SET.ACK_RESULT.ACK_PWD_ERROR){
					...
					P2PHandler.getInstance().getDeviceVersion(contact.contactId, contact.contactPassword);
				}
			}else if(intent.getAction().equals(Constants.P2P.RET_GET_DEVICE_INFO)){
				int result = intent.getIntExtra("result", -1);
				...
				deviceInfo.showDeviceInfoDialog(
						cur_version,
	...
	public void onClick(View view) {
		...
		case R.id.viewDeviceVersionBtn:
			...
			dialog.setTitle(mContext.getResources().getString(R.string.device_info));
	...
	public void replaceFragment(int type,boolean isAnim,boolean isEnforce){
		...
		try{
			...
			current_frag = type;
			transaction.replace(R.id.fragContainer, fragment, fragTags[current_frag]);
			transaction.commit();
