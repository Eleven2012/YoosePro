...
public class CutImageActivity extends BaseActivity implements OnClickListener{
	...
	public void init(){
		try{
			...
			mainBitmap = ImageUtils.getBitmap(
					new File(Constants.Image.USER_HEADER_PATH+Constants.Image.USER_HEADER_TEMP_FILE_NAME),
			...
			AbsoluteLayout.LayoutParams params1 = (android.widget.AbsoluteLayout.LayoutParams) temp.getLayoutParams();
			RelativeLayout.LayoutParams params2 = (LayoutParams) layout_bottom.getLayoutParams();
			AbsoluteLayout.LayoutParams params3 = (AbsoluteLayout.LayoutParams) layout_cut.getLayoutParams();
	...
	OnTouchListener onTouch = new OnTouchListener(){
		...
		public boolean onTouch(View view, MotionEvent event) {
			...
			AbsoluteLayout.LayoutParams params = (AbsoluteLayout.LayoutParams) layout_cut.getLayoutParams();
	...
	void checkImageSize(){
		tempBitmap = Bitmap.createBitmap(mainBitmap, 0, 0, (int)(minWidth*scale), (int)(minWidth*scale));
	...
	void updateHeader(){
		...
		try{
			...
			saveBitmap = Bitmap.createBitmap(tempBitmap, 0, 0, tempBitmap.getWidth(), tempBitmap.getHeight());
	...
	public void onClick(View v) {
		...
		case R.id.save:
			this.setResult(1);
			ImageUtils.saveImg(saveBitmap, Constants.Image.USER_HEADER_PATH+NpcCommon.mThreeNum+"/"+mContact.contactId+"/", Constants.Image.USER_HEADER_FILE_NAME);
			saveBitmap = ImageUtils.grayBitmap(saveBitmap);
			ImageUtils.saveImg(saveBitmap, Constants.Image.USER_HEADER_PATH+NpcCommon.mThreeNum+"/"+mContact.contactId+"/", Constants.Image.USER_GRAY_HEADER_FILE_NAME);
			finish();
			break;
		case R.id.back_btn:
			finish();
