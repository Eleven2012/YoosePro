...
public class AccountInfoActivity extends BaseActivity implements OnClickListener{
	...
	public void onClick(View v) {
		...
		case R.id.change_email:
			Intent modify_email = new Intent(mContext,ModifyAccountEmailActivity.class);
			startActivity(modify_email);
			break;
		case R.id.change_phone:
			Account account = AccountPersist.getInstance().getActiveAccountInfo(mContext);
			if(!(account.phone.equals("0")||account.phone.equals(""))){
				Intent unbind_phone = new Intent(mContext,UnbindPhoneActivity.class);
				startActivity(unbind_phone);
			}else{
				Intent modify_phone = new Intent(mContext,ModifyAccountPhoneActivity.class);
				startActivity(modify_phone);
			}
			break;
		case R.id.modify_login_pwd:
			Intent modify_password = new Intent(mContext,ModifyLoginPasswordActivity.class);
