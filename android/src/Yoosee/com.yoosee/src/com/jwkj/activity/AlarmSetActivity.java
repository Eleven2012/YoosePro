...
public class AlarmSetActivity extends BaseActivity implements OnClickListener{
	...
	public void initComponent(){
		int time_interval = SharedPreferencesManager.getInstance().getAlarmTimeInterval(mContext);
			...
			public void onClick(View icon,final int position) {
				// TODO Auto-generated method stub
					...
					dialog = new NormalDialog(
							mContext,
							mContext.getResources().getString(R.string.cancel_shield),
							mContext.getResources().getString(R.string.ensure_cancel_shield)+" "+alarmMask.deviceId+"?",
					...
					dialog.setOnButtonOkListener(new NormalDialog.OnButtonOkListener() {
						...
						public void onClick() {
							...
							DataManager.deleteAlarmMask(mContext, NpcCommon.mThreeNum, alarmMask.deviceId);
		...
		mList = DataManager.findAlarmMaskByActiveUser(mContext, NpcCommon.mThreeNum);
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver(){
		...
		public void onReceive(Context arg0, Intent intent) {
			if(intent.getAction().equals(Constants.Action.ADD_ALARM_MASK_ID_SUCCESS)){
	...
	public void onClick(View v) {
		...
		case R.id.setting_time:
			SharedPreferencesManager.getInstance().putAlarmTimeInterval(mContext, date_seconds.getCurrentItem()+1);
			Log.e("my",date_seconds.getCurrentItem()+"");
			time_text.setText(String.valueOf((date_seconds.getCurrentItem()+1)));
			T.showShort(mContext, R.string.modify_success);
			break;
		case R.id.add_alarm_item:
			Intent modify_alarmMaskId = new Intent(mContext,AddAlarmMaskIdActivity.class);
			mContext.startActivity(modify_alarmMaskId);
			break;
		case R.id.alarm_record:
			Intent go_alarm_record = new Intent(mContext,AlarmRecordActivity.class);
